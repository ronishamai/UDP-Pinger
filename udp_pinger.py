...
def print_summary():
    ...
    print(f"{count} packets transmitted, {fails} packets received, {(fails/count):.2f}% packet loss")

def validate_data(rcv_msg, send_msg):
    return ((rcv_msg[0] == 1) and (rcv_msg[1:] == send_msg[1:]))

def init_connection():
    pinger = socket(socket.AF_INET, socket.SOCK_DGRAM) # create an (IPV4, UDP) socket object 
...
def ping(pinger, agent_address, data):
    ...
    while (time.perf_counter() < time_window):
        ...
        if validate_data(rcv_msg, send_msg):
            ...
            print_ok_message(rtt, rcv_msg, agent)
            return 
        
    print_bad_message(count)
    fails = fails + 1
